...
function SortableContentBlock({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={cn("relative group", isDragging && "shadow-lg ring-2 ring-primary")}
    >
      <div
...
export function ContentBlockEditor({
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent, blockId: string) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  const insertLink = () => {
    ...
    if (firstContentBlock) {
      updateBlockContent(
        firstContentBlock.id,
        (firstContentBlock.content || "") + " " + markdownLink,
      );
    }
