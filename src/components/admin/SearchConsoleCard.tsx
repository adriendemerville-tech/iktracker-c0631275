...
export function SearchConsoleCard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const list = (res?.siteEntry ?? []).filter((s: any) => s.permissionLevel !== 'siteUnverifiedUser');
        setSites(list);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([sum, q, p]) => {
        const t = sum?.rows?.[0];
        setTotals(t ? { clicks: t.clicks, impressions: t.impressions, ctr: t.ctr, position: t.position } : { clicks: 0, impressions: 0, ctr: 0, position: 0 });
        setQueries(q?.rows ?? []);
        setPages(p?.rows ?? []);
      })
    ...
    <Card>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {sites.length > 0 && (
              <Select value={siteUrl} onValueChange={setSiteUrl}>
                <SelectTrigger className="h-8 w-[220px]"><SelectValue placeholder="Site" /></SelectTrigger>
                <SelectContent>
                  {sites.map((s) => (
            ...
            <ToggleGroup type="single" size="sm" value={String(days)} onValueChange={(v) => v && setDays(Number(v))}>
      ...
      <CardContent className="space-y-4">
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <Kpi icon={<MousePointer className="w-4 h-4" />} label="Clics" value={totals.clicks.toLocaleString('fr-FR')} />
              <Kpi icon={<Search className="w-4 h-4" />} label="Impressions" value={totals.impressions.toLocaleString('fr-FR')} />
              <Kpi icon={<Percent className="w-4 h-4" />} label="CTR" value={`${(totals.ctr * 100).toFixed(2)}%`} />
              <Kpi icon={<TrendingUp className="w-4 h-4" />} label="Position moy." value={totals.position.toFixed(1)} />
            </div>
...
function RowsTable({ title, rows, keyLabel, truncate }: { title: string; rows: Row[]; keyLabel: string; truncate?: boolean }) {
  return (
    <div>
      ...
      <Table>
        ...
        <TableBody>
          {rows.length === 0 ? (
            <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground text-sm">Aucune donnée</TableCell></TableRow>
          ) : rows.map((r, i) => (
            <TableRow key={i}>
              <TableCell className={truncate ? 'max-w-[240px] truncate' : ''} title={r.keys?.[0]}>{r.keys?.[0]}</TableCell>
