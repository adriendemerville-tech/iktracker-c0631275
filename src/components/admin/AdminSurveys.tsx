...
export function AdminSurveys() {
    ...
    queryFn: async () => {
      ...
      for (const s of surveys) {
        ...
        stats.push({
          ...
          unique_users_shown: new Set(imp.filter((i) => i.action === "shown").map((i) => i.user_id))
            .size,
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold">
            {editingSurvey ? "Modifier le survey" : "Nouveau survey"}
          </h2>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm((f) => ({ ...f, target_page: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              <Label className="flex items-center gap-1.5">
                Description (interne){" "}
                <span title="Non visible — admin uniquement">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, duration_days: parseInt(e.target.value) || 7 }))
                  }
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={form.max_impressions_per_user}
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      max_impressions_per_user: parseInt(e.target.value) || 3,
                    }))
                  }
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={form.delay_between_impressions_hours}
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      delay_between_impressions_hours: parseInt(e.target.value) || 24,
                    }))
                  }
                />
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <Input
                  type="number"
                  value={form.target_user_count ?? ""}
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      target_user_count: e.target.value ? parseInt(e.target.value) : null,
                    }))
                  }
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={form.target_min_days_since_signup ?? ""}
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      target_min_days_since_signup: e.target.value
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={form.target_max_days_since_signup ?? ""}
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      target_max_days_since_signup: e.target.value
                        ? parseInt(e.target.value)
                        : null,
                    }))
                  }
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">
                Variantes A/B
                {totalPct !== 100 && (
                  <span className="ml-2 text-xs text-destructive">(total: {totalPct}% ≠ 100%)</span>
                )}
              </CardTitle>
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button
            ...
            disabled={!form.title.trim() || saveMutation.isPending || totalPct !== 100}
          >
          ...
          {editingSurvey && editingSurvey.status === "draft" && (
            <Button
              ...
              onClick={() => {
                saveMutation.mutate();
                publishMutation.mutate(editingSurvey.id);
              }}
            >
  ...
  return (
    <div className="space-y-4">
        ...
        <ScrollArea className="max-h-[70vh]">
          <div className="space-y-3">
            {surveys.map((survey) => {
              ...
              const responseRate =
                stats && stats.unique_users_shown > 0
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Page : {survey.target_page} · {survey.duration_days}j · Max{" "}
                          {survey.max_impressions_per_user} affichages
                        </p>
                        {survey.published_at && (
                          <p className="text-xs text-muted-foreground mt-1">
                            Publié le{" "}
                            {format(new Date(survey.published_at), "dd MMM yyyy HH:mm", {
                              locale: fr,
                            })}{" "}
                            · en ligne depuis{" "}
                            {differenceInDays(new Date(), new Date(survey.published_at))} jours
                          </p>
                        )}
                      ...
                      <div className="flex gap-1 shrink-0">
                        ...
                        {survey.status === "published" && (
                          <Button
                            ...
                            onClick={() =>
                              pauseMutation.mutate({ id: survey.id, status: "paused" })
                            }
                          >
                        ...
                        {survey.status === "paused" && (
                          <Button
                            ...
                            title="Reprendre"
                            onClick={() =>
                              pauseMutation.mutate({ id: survey.id, status: "published" })
                            }
                          >
      ...
      >
        <DialogContent className="bg-transparent border-none shadow-none p-0 sm:max-w-none w-auto flex flex-col items-center [&>button]:hidden">
