...
const KmBarShape = (props: any) => {
  ...
  return (
    <path
      ...
      style={{
        animationDuration: `${KM_BAR_ANIMATION_DURATION_MS}ms`,
    ...
    <div className="h-48 min-h-[192px]">
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Bar dataKey="km" shape={<KmBarShape />} maxBarSize={maxBarSize}>
            {data.map((_, index) => (
              <Cell key={`cell-${index}`} fill={colors[index % colors.length]} />
            ))}
            <LabelList
              dataKey="km"
              content={(props: any) => <AnimatedLabel {...props} isMobile={isMobile} />}
            />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
