...
export function CalendarSyncNotification({
  dateRange,
  tripsCreated,
  onClose,
}: CalendarSyncNotificationProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs">
      <Card className="relative mx-4 max-w-md bg-muted/95 p-6 shadow-xl">
        ...
        <div className="space-y-3 text-center">
          <p className="text-lg font-medium text-foreground">Import terminé</p>
          <p className="text-sm text-muted-foreground">
            {tripsCreated > 0 ? (
              <>
                <span className="font-semibold text-foreground">
                  {tripsCreated} trajet{tripsCreated > 1 ? "s" : ""}
                </span>{" "}
                importé{tripsCreated > 1 ? "s" : ""} depuis votre calendrier
              </>
            ) : (
          ...
          <p className="text-xs text-muted-foreground">
            Du <span className="font-medium">{formatDate(dateRange.startDate)}</span> au{" "}
