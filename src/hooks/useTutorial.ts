  ...
  useEffect(() => {
    ...
    const incrementAndCheck = async () => {
      try {
        ...
        await supabase
          .from("user_preferences")
          .upsert({ user_id: user.id, visit_count: newCount } as any, { onConflict: "user_id" });
  ...
  const completeTutorial = useCallback(async () => {
    ...
    if (user) {
      try {
        await supabase
          .from("user_preferences")
          .upsert({ user_id: user.id, tutorial_completed_at: new Date().toISOString() } as any, {
            onConflict: "user_id",
          });
      } catch (e) {
  ...
  const resetTutorial = useCallback(async () => {
    ...
    if (user) {
      try {
        await supabase
          .from("user_preferences")
          .upsert({ user_id: user.id, tutorial_completed_at: null } as any, {
            onConflict: "user_id",
          });
      } catch (e) {
