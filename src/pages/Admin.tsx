...
const Admin = () => {
  ...
  const deleteConversation = useMutation({
    mutationFn: async (userId: string) => {
      const { error } = await supabase.from("feedback").delete().eq("user_id", userId);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-feedbacks"] });
  ...
  const users: UserWithRole[] = searchedUsers.map((u) => {
    ...
    const lastActivity =
      userFeedbacks.length > 0
        ? userFeedbacks.reduce(
            (latest, f) => (new Date(f.created_at) > new Date(latest) ? f.created_at : latest),
    ...
    return {
      ...
      userRole:
        (userRoles.find((r) => r.user_id === u.user_id)?.role as "admin" | "viewer") || null,
      feedbackCount,
      lastActivity,
  ...
  const getRandomClosing = () =>
    CLOSING_FORMULAS[Math.floor(Math.random() * CLOSING_FORMULAS.length)];
  ...
  const conversations: ConversationGroup[] = (() => {
    ...
    return Array.from(grouped.entries())
      .map(([userId, msgs]) => {
        ...
        const name =
          latest.user_first_name || latest.user_last_name
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background cursor-default">
                ...
                {unresolvedErrors > 0 && (
                  ...
                  >
                    ...
                    <span className="text-xs font-semibold">
                      {unresolvedErrors} erreur{unresolvedErrors > 1 ? "s" : ""}
                    </span>
                  </button>
                )}
                          ...
                          {conversations.map((convo) => (
                            ...
                            >
                              ...
                              <div className="flex items-start justify-between gap-2 mb-1.5 pr-6">
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                  ...
                                  {convo.unrespondedCount > 0 ? (
                                    ...
                                    <Badge
                                      variant="outline"
                                      className="text-green-600 border-green-600 text-[10px] h-5"
                                    >
                              ...
                              <div className="flex items-center gap-2 mt-1.5 text-xs text-muted-foreground">
                                ...
                                {format(new Date(convo.lastMessageAt), "dd MMM yyyy HH:mm", {
                                  locale: fr,
                                })}
                              </div>
                            </div>
                          ...
                          <div className="space-y-4 pr-2">
                            {selectedConversation.messages.map((msg, idx) => (
                              <div key={msg.id} className="space-y-2">
                                {/* Message bubble */}
                                <div
                                  className={`rounded-lg p-3 ${msg.is_admin_message ? "bg-primary/10 ml-4 border-l-2 border-primary" : "bg-muted/50"}`}
                                >
                                  <div className="flex items-center justify-between mb-1.5">
                                    <p
                                      className={`text-xs font-medium flex items-center gap-1 ${msg.is_admin_message ? "text-primary" : "text-muted-foreground"}`}
                                    >
                                  ...
                                  {msg.device_info && (
                                    <div className="mt-2 flex items-center gap-1.5 text-[10px] text-muted-foreground">
                                      {msg.device_info.platform === "mobile" ? (
                                        <Smartphone className="w-3 h-3" />
                                      ) : (
                                        <Monitor className="w-3 h-3" />
                                      )}
                                      <span>
                                        {msg.device_info.os} · {msg.device_info.browser}
                                      </span>
                                    </div>
                                  )}
                                </div>
                                  ...
                                  <div className="bg-primary/10 rounded-lg p-3 ml-4 border-l-2 border-primary">
                                    ...
                                    {msg.responded_at && (
                                      <p className="text-[10px] text-muted-foreground mt-1.5">
                                        {format(new Date(msg.responded_at), "dd MMM yyyy à HH:mm", {
                                          locale: fr,
                                        })}
                                      </p>
                                    )}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="flex gap-2 mb-4">
                    ...
                    <Button onClick={() => setDebouncedSearch(userSearch)} variant="outline">
                                ...
                                {(() => {
                                  ...
                                  if (personaOption) {
                                    ...
                                    return (
                                      <Badge
                                        variant="outline"
                                        className="flex-shrink-0 text-[10px] px-1.5 py-0 gap-0.5"
                                      >
                              ...
                              {u.isAdmin ? (
                                <div className="flex items-center gap-1">
                                  ...
                                  <Button
                                    ...
                                    disabled={
                                      removeAdminMutation.isPending || u.user_id === user?.id
                                    }
        ...
        <AlertDialog
          open={!!convoToDelete}
          onOpenChange={(open) => !open && setConvoToDelete(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
