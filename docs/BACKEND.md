...
**Cron** : `content-freshness-audit-daily`, tous les jours à `0 6 * * *` (UTC), corps `{"checkLinks": true}` (remplace l'ancien `content-freshness-audit-weekly`). Durée observée ~50 s pour 100 articles. Les articles sont analysés **par lots parallèles de 5**, et les statuts de liens sont mis en cache dans la table `public.link_status_cache` (`url` PK, `status`, `checked_at`) avec une validité de **7 jours**, partagée entre deux runs.
