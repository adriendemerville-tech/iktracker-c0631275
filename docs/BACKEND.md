...
- **Modérateur** : colonne `is_moderator BOOLEAN NOT NULL DEFAULT false` sur `public.forum_profiles` (activée pour Adrien de Volontat). Badge « Modérateur » dans les listes et la fiche contributeur ; bouton « Écrire au modérateur » qui insère le message dans `public.feedback` (préfixe `[Forum → modérateur]`) et le fait remonter dans l'onglet **Avis** de `/admin`.
...
- **Consolidation blog — cannibalisation clusters 1-4 (28 août 2026)** : audit de similarité des titres/slugs sur 167 URLs → **44 articles supplémentaires** passés en `status='archived'` + `is_listed=false` et redirigés en 301 vers leur pilier (`/blog/controle-urssaf-frais-kilometriques-2026` pour les clusters « sécuriser ses IK / anti-redressement URSSAF » et « 7 règles d'or », `/indemnites-kilometriques` pour le cluster « calcul des indemnités », `/blog/7-erreurs-courantes-indemnite-kilometrique-a-eviter` pour le cluster « 7 erreurs »). Les 3 miroirs totalisent désormais **68 slugs** (`scripts/validate-blog-redirects-sync.cjs` OK). Tous les slugs archivés sont inscrits dans `blog_slug_blacklist` (`is_pattern=false`) pour bloquer toute republication automatique via `blog-api` (409 `slug_in_trash`). Edge functions `meta-renderer` et `sitemap` redéployées.
...
- **Rotation / scope éditorial** : 21 topics étiquetés par `format` (`video` / `carousel`) et par `mediaSource` (`browserless` / `wavespeed`), répartis en **6 familles** — `PRODUCT_SLUGS` (fonctionnalités, 7), `WORKFLOW_SLUGS` (workflows transversaux, 2), `PROBLEM_SLUGS` (problèmes visés, 2), `BLOG_SLUGS` (articles de blog, 1 générique résolu dynamiquement), `PRICING_SLUGS` (tarifs, 1), `LEAD_MAGNET_SLUGS` (lead magnets, 3), plus `CONTEXT_SLUGS` (contexte/valeurs, 5). `FAMILY_CYCLE` fixe l'ordre sur 10 mois : produit, problème, produit, blog, workflow, produit, lead magnet, contexte, produit, tarifs. `pickTopicForThisMonth(now, recentSlugs)` calcule `n = année × 12 + mois`, sélectionne `FAMILY_CYCLE[n % 10]` puis un sujet cyclique du pool en sautant les slugs récents. `resolveBlogTopic()` remplace le slug `article-blog` par un article réellement publié (`blog_posts`, `status = 'published'`, le plus récent non encore relayé) en réécrivant `title`, `url` et `focus`.
- **Anti-redondance (historique)** : avant toute génération, `fetchPostHistory()` lit les 12 derniers runs publiés de `public.linkedin_post_log` (`status = 'success'`, `linkedin_post_id` non nul). Les slugs récents sont passés à `pickTopicForThisMonth()`, qui avance dans le pool tant que le sujet a déjà été publié dans la fenêtre récente (taille du pool − 1) : aucun module n'est repris tant que les autres n'ont pas été couverts. En parallèle, `historyPromptBlock()` injecte dans le prompt les 6 derniers posts (date, module, première ligne/hook) avec la consigne de ne réutiliser ni les angles, ni les hooks, ni les exemples chiffrés déjà publiés. Un `?topic=<slug>` forcé court-circuite la rotation mais garde le bloc anti-redondance.
...
- **Ancrage sur la documentation technique** : `supabase/functions/linkedin-weekly-post/docs-context.ts` est **généré** depuis `docs/BACKEND.md` + `docs/FRONTEND.md` par `scripts/generate-linkedin-docs-context.cjs` (blocs de code retirés, 1 600 signes max par section). À l'exécution, `docContextForTopic()` sélectionne les 4 sections les plus pertinentes par score de mots-clés (titre pondéré ×3, seuil ≥ 2, table `DOC_KEYWORDS` par slug) et les injecte, plafonnées à 4 500 signes, dans le prompt texte et dans le prompt de plan de carrousel (2 500 signes). Le prompt interdit explicitement de citer des noms de tables, de fonctions ou de fournisseurs d'infrastructure. **Relancer le script après toute évolution majeure de la doc.**
...
- **Média** :
  - `mediaSource: 'browserless'` → **vraie vidéo MP4 de l'UI via PageBolt** (`capturePageboltVideo`, `POST https://pagebolt.dev/api/v1/video`, header `x-api-key: PAGEBOLT_API_KEY`). Steps : **scénario scripté** (`scriptedVideoSteps`) — `navigate`, `evaluate` de `scrollIntoView` sur l'ancre du module, puis pour le topic `simulateur` une saisie réelle `fill input[id^='annualKm'] = 12000` et un `evaluate` qui active le switch `[id^='electric']` (majoration 20%), avec `wait { live: true }` entre chaque action et deux `scroll` relatifs de fin. En cas d'échec du scénario, repli automatique sur `fallbackVideoSteps` (défilement par positions absolues 700 / 1500 / 2400). Viewport 1280×720, 30 fps, curseur `highlight`, `response_type: 'json'` (MP4 base64, ~2,2 Mo / ~23 s). Coût : 3 requêtes de quota par vidéo. Le MP4 est publié en post VIDEO. Repli 1 : **carrousel PDF de vraies captures d'écran** (`captureUiFrames` + `renderScreenshotCarouselPdf`) — le runtime Browserless `/function` s'exécute côté navigateur (ni `fs`, ni `child_process`, ni `ffmpeg`, `page.screencast` indisponible), il navigue, réapplique le viewport 1200×1200 puis recharge, repère les ancres de focus dérivées du texte et renvoie 5 PNG assemblés en PDF publié en post DOCUMENT. Repli 2 : capture unique publiée en image.
...
- `/independants` : `priority 0.8`, `monthly`
...
- **Edge function** `generate-recurring-trips` : insère un trip dans `trips` (source='recurring') pour chaque récurrence active dont `days_of_week` contient le jour courant (UTC), filtrée par `active_months` et la fenêtre `weeks_duration` depuis `created_at`. Calcule `ik_amount` via le barème embarqué + bonus 20% EV. **Auth obligatoire** (15/08/2026) : Bearer = `SUPABASE_SERVICE_ROLE_KEY`, `RECURRING_TRIPS_CRON_TOKEN`, ou tout JWT machine (`role` = `anon`/`service_role`, signature déjà validée par le gateway `verify_jwt`) fourni en `Authorization` ou `apikey` — c'est le pattern pg_cron standard ; sinon header `x-cron-secret` = `CRON_SECRET`/`RECURRING_TRIPS_CRON_TOKEN`. Les JWT utilisateurs sont refusés.
...
- **3.9** (3 août 2026) — Correctif blog public : la table `public.blog_posts` n'avait **aucun GRANT** (Data API inaccessible) et la policy admin, non restreinte à `authenticated`, faisait échouer `has_role` pour le rôle `anon` (`42501 permission denied for function has_role`) — résultat : blog vide pour tous les visiteurs malgré 75 articles publiés. Ajout de `GRANT SELECT` (anon) / CRUD (authenticated) / `ALL` (service_role) sur `blog_posts`, GRANTs sur `blog_slug_blacklist` (authenticated + service_role) et `blog_api_keys` (service_role seul). Policies `Admins can manage all posts` et `Anyone can read published posts` recréées avec clause `TO` explicite.
...
- UI : badge « Forum » dans la sidebar desktop, cloche `ForumNotificationsBell` dans le header de `/app` (mobile + desktop) et dans `MarketingNav` (donc sur `/forum` et toutes les pages publiques). Le composant ne rend rien pour un visiteur non connecté.
- Correctifs 29/08/2026 : triggers `trg_forum_notify_on_discussion` et `trg_forum_notify_on_reply` passés en `AFTER INSERT OR UPDATE OF status` (une publication différée depuis un brouillon / la modération notifie désormais), garde anti-doublon sur `reply_id`, et détection des mentions en Unicode (`@([\w\-\.]{2,40})`) pour les pseudos accentués.
- Temps réel : `public.forum_notifications` ajoutée à la publication `supabase_realtime` (`REPLICA IDENTITY FULL`). Le hook `useForumNotifications` s'abonne aux INSERT filtrés par `user_id` et calcule le nombre de non-lues via un `count: 'exact'` serveur (plus de plafond à 20).
