...
async function sendResend(params: {
  ...
  if (!res.ok)
    throw new Error(`resend send failed [${res.status}]: ${(await res.text()).slice(0, 500)}`);
...
Deno.serve(async (req) => {
  ...
  if (overrideEmail && !auth.privileged) {
    console.warn(
      "[send-user-monthly-report] override_email ignored for non-privileged caller",
      auth.callerId,
    );
    overrideEmail = null;
  }
  ...
  for (const p of (prefs ?? []) as Array<{
    ...
    try {
      ...
      if (!onlyUserId && p.user_monthly_report_last_sent_at) {
        const days =
          (today.getTime() - new Date(p.user_monthly_report_last_sent_at).getTime()) / 86400000;
        if (days < 25) {
      ...
      const periodTrips = trips.filter(
        (t) => t.date >= isoDay(period.start) && t.date < isoDay(period.end),
      );
      ...
      await sendResend({
        ...
        html: buildEmailHtml({
          ...
          ytdLabel: ytd.label,
          monthCount: periodTrips.length,
          monthKm: totalKm(periodTrips),
          monthIk: totalIk(periodTrips),
          ytdCount: ytdTrips.length,
          ytdKm: totalKm(ytdTrips),
          ytdIk: totalIk(ytdTrips),
          monthUrl,
          ytdUrl,
        }),
        attachments: [
          {
            filename: `IKtracker-${period.label.replace(/\s+/g, "-")}.pdf`,
            content: toBase64(monthPdf),
          },
