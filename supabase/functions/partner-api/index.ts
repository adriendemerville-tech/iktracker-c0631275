...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-api-key, x-external-user-id",
};
...
async function authenticatePartner(
  req: Request,
): Promise<PartnerContext | { error: string; status: number }> {
...
async function findOrCreateIktrackerUser(
  partnerId: string,
  externalUserId: string,
  ...
  if (existingUser) {
    ...
    if (createErr || !created.user)
      throw new Error(`Failed to provision user: ${createErr?.message}`);
  ...
  fireWebhook(partnerId, "user.linked", {
    external_user_id: externalUserId,
    iktracker_user_id: iktrackerUserId,
    email: externalEmail,
  });
...
function calculateIk({ fiscalPower, isElectric, annualKm, tripKm }: IkInput) {
  ...
  const scale: Record<
    number,
    { low: (km: number) => number; mid: (km: number) => number; high: (km: number) => number }
  > = {
    3: { low: (km) => km * 0.529, mid: (km) => km * 0.316 + 1065, high: (km) => km * 0.37 },
    4: { low: (km) => km * 0.606, mid: (km) => km * 0.34 + 1330, high: (km) => km * 0.407 },
    5: { low: (km) => km * 0.636, mid: (km) => km * 0.357 + 1395, high: (km) => km * 0.427 },
    6: { low: (km) => km * 0.665, mid: (km) => km * 0.374 + 1457, high: (km) => km * 0.447 },
    7: { low: (km) => km * 0.697, mid: (km) => km * 0.394 + 1515, high: (km) => km * 0.47 },
  };
  ...
  return {
    ...
    bracket,
    bracketLabel:
      bracket === "low" ? "≤ 5 000 km" : bracket === "mid" ? "5 001 – 20 000 km" : "> 20 000 km",
    electricBonus: isElectric ? 0.2 : 0,
...
async function handleCreateTrip(req: Request, ctx: PartnerContext): Promise<Response> {
  ...
  if (
    ...
    return jsonResponse(
      { error: "Required: external_email, date, start_location, end_location, distance" },
      400,
    );
  }

  const userId = await findOrCreateIktrackerUser(
    ctx.partnerId,
    externalUserId,
    external_email,
    body.metadata || {},
  ...
  if (effectiveVehicleId) {
    ...
    if (veh) {
      ...
      ikAmount = calculateIk({
        fiscalPower: veh.fiscal_power,
        isElectric: veh.is_electric,
        annualKm,
        tripKm: distance,
      }).tripIkAmount;
    }
...
async function handleListTrips(req: Request, ctx: PartnerContext): Promise<Response> {
  ...
  let query = admin
    .from("trips")
    .select(
      "id, date, start_location, end_location, distance, ik_amount, vehicle_id, purpose, round_trip, tour_stops, status, source, created_at",
    )
    .eq("user_id", userId)
    .is("deleted_at", null);
...
async function handleUpdateTrip(
  ...
  if (Object.keys(patch).length === 0)
    return jsonResponse({ error: "Nothing to update (supported: purpose)" }, 400);
  ...
  fireWebhook(ctx.partnerId, "trip.updated", {
    trip_id: data.id,
    external_user_id: externalUserId,
...
async function buildReportHtml(
  ...
  partnerName: string,
): Promise<{ html: string; totalKm: number; totalIk: number; tripsCount: number }> {
...
async function handleGenerateReport(req: Request, ctx: PartnerContext): Promise<Response> {
  if (
    !requireScope(ctx, "reports") &&
    !requireScope(ctx, "trips:read") &&
  ...
  if (!userId)
    return jsonResponse({ error: "User not linked. Call /sso/magic-link or /trips first." }, 404);
  ...
  const expiresAt = new Date(
    Date.now() + Math.max(1, Math.min(90, expires_in_days)) * 86400 * 1000,
  ).toISOString();
...
async function handleUpdatePreferences(req: Request, ctx: PartnerContext): Promise<Response> {
  ...
  if (body.calendar_import_mode !== undefined) {
    if (!VALID_CAL_MODES.includes(body.calendar_import_mode)) {
      return jsonResponse(
        { error: `calendar_import_mode must be one of: ${VALID_CAL_MODES.join(", ")}` },
        400,
      );
    }
  ...
  if (body.ik_rate_override !== undefined) {
    if (!VALID_IK_OVERRIDES.includes(body.ik_rate_override)) {
      return jsonResponse(
        { error: `ik_rate_override must be one of: ${VALID_IK_OVERRIDES.join(", ")}` },
        400,
      );
    }
...
async function handleUpdateVehicle(
  ...
  if ((fiscalChanged || electricChanged) && update_past_trips === true) {
    ...
    for (const trip of vehTrips ?? []) {
      ...
      const newIkAmount =
        Math.round((ik.annualIkAmount - prev.annualIkAmount) * (newIsElectric ? 1.2 : 1) * 100) /
        100;
...
serve(async (req) => {
  ...
  if (route === "/internal/preferences-changed" && req.method === "POST") {
    try {
      return await handleInternalPreferencesChanged(req);
    } catch (e) {
      return jsonResponse({ error: (e as Error).message }, 500);
    }
  }
  ...
  try {
    ...
    if (route === "/health" && req.method === "GET") {
      res = jsonResponse({
        ...
        quota_remaining: ctx.quotaRemaining,
      });
    ...
    await logRequest(
      ...
      res.status,
      Date.now() - start,
      req.headers.get("x-external-user-id") ?? undefined,
    );
    return res;
  } catch (e) {
