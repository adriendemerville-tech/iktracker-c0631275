...
serve(async (req) => {
  ...
  const isCron =
    ...
    (!!providedCronSecret &&
      (providedCronSecret === cronSecret || providedCronSecret === altCronSecret));
  ...
  for (let offset = 0; offset < 20000; offset += PAGE) {
    let query = supabase
      .from("trips")
      .select(
        "id, user_id, vehicle_id, date, start_location, end_location, start_address, end_address, start_lat, start_lng, end_lat, end_lng, distance, ik_amount, round_trip, tour_stops, status",
      )
      .is("deleted_at", null)
      .gte("date", sinceDate)
  ...
  async function annualKmBefore(
    ...
    const { data } = await supabase
      ...
      .lt("date", tripDate);
    return (data || []).reduce(
      (sum: number, t: { distance: number | null }) => sum + (Number(t.distance) || 0),
      0,
    );
  }
  ...
  for (const trip of rows) {
    try {
      ...
      const sameEndpoints =
        !isLoopTour && !!normalize(startText) && normalize(startText) === normalize(endText);
      ...
      if (needsGeoRecalc) {
        if (sameEndpoints) {
          ...
          if (
            !origin ||
            normalize(origin).length < 4 ||
            /^(maison|domicile|position)$/.test(normalize(origin))
          ) {
      ...
      if (distanceChanged || ikMissing) {
        ...
        if (trip.vehicle_id && newDistance > 0) {
          ...
          if (vehicle) {
            ...
            expectedIk =
              annualIK(after, vehicle.fiscal_power) - annualIK(before, vehicle.fiscal_power);
  ...
  console.log(
    "trips-guard done",
    JSON.stringify({ scanned: rows.length, fixed, skipped, failed, geoCalls }),
  );
