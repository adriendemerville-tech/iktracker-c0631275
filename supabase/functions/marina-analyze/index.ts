...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    try {
      const response = await fetch(`https://crawlers.fr/functions/v1/marina?job_id=${encodeURIComponent(jobId)}`, {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const response = await fetch('https://crawlers.fr/functions/v1/marina', {
        method: 'POST',
        headers: {
