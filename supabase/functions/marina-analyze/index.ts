...
Deno.serve(async (req) => {
  ...
  if (!marinaKey) {
    return new Response(JSON.stringify({ error: "Marina API key not configured" }), {
      status: 500,
  ...
  if (req.method === "GET") {
    ...
    try {
      const response = await fetch(
        `https://tutlimtasnjabdfhpewu.supabase.co/functions/v1/marina?job_id=${encodeURIComponent(jobId)}`,
        {
      ...
      return new Response(JSON.stringify({ error: "Failed to poll job status" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
