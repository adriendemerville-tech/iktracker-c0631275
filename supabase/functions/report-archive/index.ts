...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'generate_annual') {
      ...
      const userName = [meta.first_name, meta.last_name].filter(Boolean).join(' ') || user.email || 'Utilisateur'

      const title = `Relevé kilométrique — ${label}`
      const html = wrapForPdf(title, buildReportBody(title, userName, tripList, (vehicles ?? []) as ReportVehicle[]))
