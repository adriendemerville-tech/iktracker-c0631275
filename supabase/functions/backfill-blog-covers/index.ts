...
function buildPrompt(title: string): string {
  ...
  if (t.includes('urssaf') || t.includes('contrôle') || t.includes('redressement') || t.includes('fiscal') || t.includes('impôt'))
    scene = 'an organised desk with official French tax paperwork, a calculator, a laptop and a cup of coffee, natural window light';
  ...
  if (t.includes('tournée') || t.includes('tournee') || t.includes('artisan') || t.includes('chantier'))
    scene = 'a craftsman utility car parked near a worksite in a French village, early morning light';
  if (t.includes('kilométr') || t.includes('trajet') || t.includes('gps') || t.includes('suivi'))
    scene = 'a car dashboard with an odometer and a smartphone mounted on the windshield, French countryside road ahead';
...
async function generateImage(prompt: string): Promise<string> {
  ...
  while ((!outputs || outputs.length === 0) && requestId && status !== 'failed' && Date.now() - start < 90_000) {
  ...
  for (const post of batch) {
    try {
      ...
      const { error: upErr } = await admin.storage
        .from('blog-images')
        .upload(filename, bytes, { contentType: 'image/jpeg', cacheControl: '31536000', upsert: false });
      ...
      results.push({ id: post.id, slug: post.slug, error: e instanceof Error ? e.message : String(e) });
    }
  }
