...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const selected: [string, string][] =
      Array.isArray(bots) && bots.length
        ? bots.filter((b: string) => DEFAULT_BOTS[b]).map((b: string) => [b, DEFAULT_BOTS[b]])
        : Object.entries(DEFAULT_BOTS);

    const results = await Promise.all(
      selected.map(([name, ua]) => testBot(target.toString(), name, ua)),
    );

    return new Response(
      JSON.stringify({ url: target.toString(), testedAt: new Date().toISOString(), results }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
    );
