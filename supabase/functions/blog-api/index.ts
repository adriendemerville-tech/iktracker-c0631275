...
async function handleSeo(
  ...
  if (subResource === "sitemap") {
    ...
    if (req.method === "PUT") {
      ...
      const { data, error } = await supabase
        ...
        .upsert(
          {
            config_key: "sitemap_config",
            content: JSON.stringify(body.config ?? {}),
            updated_at: new Date().toISOString(),
            updated_by: "api",
          },
          { onConflict: "config_key" },
  ...
  if (subResource === "redirects") {
    ...
    if (req.method === "POST") {
      const body = await req.json();
      if (!body.source_path || !body.target_url)
        return errorResp("source_path and target_url are required", 400);
      const { data, error } = await supabase
        .from("seo_redirects")
        .insert({
...
async function handleInjection(
  ...
  if (subResource === "head" || subResource === "body-end") {
    ...
    if (req.method === "PUT") {
      ...
      const items = Array.isArray(body.injections)
        ? body.injections
        : [{ content: body.content ?? "", label: body.label ?? location }];
  ...
  if (subResource === "page" && subId) {
    ...
    if (req.method === "PUT") {
      ...
      const items = Array.isArray(body.injections)
        ? body.injections
        : [{ content: body.content ?? "", label: body.label ?? subId }];
      const toInsert = items.map((item: any) => ({
  ...
  if (subResource === "navigation") {
    if (req.method === "GET") {
      const { data } = await supabase
        .from("site_config")
        .select("*")
        .in("config_key", ["navigation_header", "navigation_footer"]);
  ...
  if (req.method === "POST") {
    ...
    if (
      ...
      if (!body.filename || !body.data)
        return errorResp("filename and data (base64) required", 400);
      ...
      .upsert(
        {
          ...
          content: body.content ?? {},
          schema_org: body.schema_org ?? null,
...
async function handlePosts(
  ...
  if (req.method === "GET") {
    if (isPublic) {
      ...
      const { data, error, count } = await supabase
        .from("blog_posts")
        .select(
          "id, slug, title, subtitle, meta_description, featured_image_url, author_name, published_at, created_at",
          { count: "exact" },
        )
        .eq("status", "published")
        .order("published_at", { ascending: false })
  ...
  if (req.method === "POST") {
        ...
        return jsonResp(
          {
            ...
            message:
              "Non, ce contenu existe déjà (ou a été retiré volontairement). Le slug est en liste noire — ne le republiez pas. Choisissez un autre angle/slug ou contactez l'admin.",
            slug: postSlug,
          },
          409,
        );
      ...
      return jsonResp(
        {
          ...
          error: "slug_in_trash",
          message:
            "Non, ce contenu existe déjà (dans la corbeille). Restaurez-le depuis l'admin ou choisissez un autre slug.",
          slug: postSlug,
        },
        409,
      );
      ...
      return successResp(
        {
          ...prevData,
          _skipped: true,
          _reason: 'slug_already_exists. Use "force": true to overwrite.',
        },
        200,
      );
    }
    const subtitleResolved = resolveField(body, [
      "subtitle",
      "subTitle",
      "sub_title",
      "sousTitre",
    ...
    const metaResolved = resolveField(body, [
      "meta_description",
      "metaDescription",
      "meta",
      "seoDescription",
    ...
    const authorResolved = resolveField(body, [
      "author_name",
      "authorName",
      "author",
      "auteur",
...
async function handleAutopilot(
  ...
  if (req.method === "GET" && subResource === "health") {
    const { data: activeEvents } = await supabase
      .from("autopilot_events")
      .select("page_key, severity")
      .eq("resolved", false);

    const pageHealth: Record<
      string,
      { critical: number; warning: number; info: number; score: number }
    > = {};
    for (const evt of activeEvents || []) {
      const key = evt.page_key || "__global__";
      if (!pageHealth[key]) pageHealth[key] = { critical: 0, warning: 0, info: 0, score: 100 };
      if (evt.severity === "critical") {
        pageHealth[key].critical++;
        pageHealth[key].score = 0;
      } else if (evt.severity === "warning") {
        pageHealth[key].warning++;
        pageHealth[key].score = Math.min(pageHealth[key].score, 50);
      } else {
        pageHealth[key].info++;
      }
    }
    ...
    endpoints: {
      ...
      posts: {
        statuses: {
          ...
          published: "Publié — visible sur /blog et indexable",
          archived: "Archivé — masqué de /blog mais conservé, accessible via son URL si is_listed",
          deleted:
            "Corbeille (soft-delete) — masqué partout, restaurable. Slug bloqué à la recréation.",
        },
        "GET /posts": {
          auth: false,
          description: "List published posts only (excludes draft, archived, deleted)",
          params: "limit, offset",
        },
        "GET /posts/:slug": {
          auth: false,
          description: "Get a published post by slug (404 if not published)",
        },
        "GET /posts?all=true": {
          auth: true,
          description: "List ALL posts including draft, archived AND deleted (trash)",
        },
        "GET /posts?status=archived": { auth: true, description: "List archived posts only" },
        "GET /posts?status=deleted": {
          auth: true,
          description: "List trashed posts only (soft-deleted)",
        },
        "GET /posts?status=draft": { auth: true, description: "List drafts only" },
        "POST /posts": {
          auth: true,
          description:
            "Create or update a post (upsert by slug). Refuses slugs in trash or blacklist (409).",
          body: "title*, slug*, content, status (draft|published|archived), meta_description, featured_image_url/image_url, author_name, subtitle, published_at, force (bool to overwrite existing)",
        },
        "PUT /posts/:slug": {
          auth: true,
          description:
            'Update a post by slug. Set status to "archived" to archive, or "published" to restore from trash/archive.',
        },
        "DELETE /posts/:slug": {
          auth: true,
          description:
            "Soft-delete: moves post to trash (status=deleted, sets deleted_at). Reversible via PUT with status=published/archived/draft.",
        },
        "DELETE /posts/:slug?hard=true": {
          auth: true,
          description: "PERMANENT purge — irreversible deletion from DB. Use with caution.",
        },
      },
      pages: {
        ...
        "POST /pages": {
          auth: true,
          description: "Create or update a page (upsert by page_key)",
          body: "page_key*, title, meta_title, meta_description, content, schema_org, canonical_url",
        },
      ...
      injection: {
        ...
        "PUT /injection/head": {
          auth: true,
          description: "Set global head injections",
          body: "content, label, is_active OR injections[]",
        },
        ...
        "GET /injection/page/:page_key": {
          auth: true,
          description: "Get page-specific injections",
        },
        "PUT /injection/page/:page_key": {
          auth: true,
          description: "Set page-specific injections",
        },
      },
      seo: {
        ...
        "POST /seo/redirects": {
          ...
          body: "source_path*, target_url*, status_code (301|302), is_active",
        },
      ...
      media: {
        ...
        "POST /media": {
          auth: true,
          description: "Upload a file (multipart or base64 JSON)",
          body: "filename*, data* (base64), content_type",
        },
      ...
      audit: {
        ...
        "POST /audit/revert/:log_id": {
          auth: true,
          description: "Revert a change by audit log ID",
        },
      },
      autopilot: {
        "GET /autopilot/registry": {
          auth: true,
          description: "Timeline of changes with linked events",
          params: "limit, offset, page_key",
        },
        "GET /autopilot/health": { auth: true, description: "Per-page health scores (0-100)" },
        "GET /autopilot/events": {
          auth: true,
          description: "List autopilot events",
          params: "resolved (true|false)",
        },
        "POST /autopilot/events": {
          auth: true,
          description: "Report a detected event/bug",
          body: "message*, event_type, severity (info|warning|critical), page_key, details, audit_log_id",
        },
      },
    },
    field_aliases: {
    ...
    if (!isWebhookAuth) {
      ...
      if (isWriteOp) {
        ...
        if (currentUsage >= (keyData.monthly_quota ?? 10000)) {
          ...
          return errorResp(
            `Monthly quota exceeded (${keyData.monthly_quota} write requests/month)`,
            429,
          );
        }
