    ...
    endpoints: {
      ...
      posts: {
        statuses: {
          ...
          deleted: 'Corbeille (soft-delete) — masqué partout, restaurable. Slug bloqué à la recréation.',
        },
        'GET /posts': { auth: false, description: 'List published posts only (excludes draft, archived, deleted)', params: 'limit, offset' },
        'GET /posts/:slug': { auth: false, description: 'Get a published post by slug (404 if not published)' },
        'GET /posts?all=true': { auth: true, description: 'List ALL posts including draft, archived AND deleted (trash)' },
        'GET /posts?status=archived': { auth: true, description: 'List archived posts only' },
        'GET /posts?status=deleted': { auth: true, description: 'List trashed posts only (soft-deleted)' },
        'GET /posts?status=draft': { auth: true, description: 'List drafts only' },
        'POST /posts': { auth: true, description: 'Create or update a post (upsert by slug). Refuses slugs in trash or blacklist (409).', body: 'title*, slug*, content, status (draft|published|archived), meta_description, featured_image_url/image_url, author_name, subtitle, published_at, force (bool to overwrite existing)' },
        'PUT /posts/:slug': { auth: true, description: 'Update a post by slug. Set status to "archived" to archive, or "published" to restore from trash/archive.' },
        'DELETE /posts/:slug': { auth: true, description: 'Soft-delete: moves post to trash (status=deleted, sets deleted_at). Reversible via PUT with status=published/archived/draft.' },
        'DELETE /posts/:slug?hard=true': { auth: true, description: 'PERMANENT purge — irreversible deletion from DB. Use with caution.' },
      },
      pages: {
