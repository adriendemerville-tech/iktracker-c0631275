...
DECLARE
  ...
  total_simulations bigint; total_signup_clicks bigint; total_crawlers_clicks bigint;
...
BEGIN
  ...
  SELECT COUNT(*) INTO total_crawlers_clicks FROM public.marketing_analytics ma
  WHERE ma.event_type = 'crawlers_click' AND ma.created_at >= start_date
    AND (ma.user_id IS NULL OR NOT EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = ma.user_id AND ur.role = 'admin'))
    AND (ma.ip_address IS NULL OR NOT EXISTS (SELECT 1 FROM public.excluded_ips ei WHERE ei.ip_address = ma.ip_address));
