...
BEGIN
  ...
  -- Symmetric filter: exclude admins AND users whose signup events came from
  -- an excluded IP (office / internal test traffic). Otherwise the numerator
  -- (new_users) systematically exceeds the denominator (views).
  ...
  WHERE u.created_at >= start_date
    ...
    AND NOT EXISTS (
      SELECT 1
      FROM public.marketing_analytics ma
      JOIN public.excluded_ips ei ON ei.ip_address = ma.ip_address
      WHERE ma.user_id = u.id
        AND ma.event_type IN ('signup_success', 'signup_form_submit', 'signup_oauth_start')
    );
  ...
  result := json_build_object(
    ...
    'top_errors', (
      SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json) FROM (
        SELECT referrer AS message, COUNT(*) AS count
        FROM public.marketing_analytics ma
        WHERE ma.created_at >= start_date
          AND ma.page IN ('signup', 'auth')
