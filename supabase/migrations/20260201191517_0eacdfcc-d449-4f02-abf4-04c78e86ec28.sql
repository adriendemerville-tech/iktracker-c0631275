-- Update get_user_stats to include trip source breakdown
...
BEGIN
  ...
  IF user_trips_count > 0 THEN
    calendar_pct := ROUND((calendar_trips_count::float / user_trips_count::float) * 100, 1);
    manual_pct := ROUND((manual_trips_count::float / user_trips_count::float) * 100, 1);
    tour_pct := ROUND((tour_trips_count::float / user_trips_count::float) * 100, 1);
  ELSE
    calendar_pct := 0;
    manual_pct := 0;
    tour_pct := 0;
