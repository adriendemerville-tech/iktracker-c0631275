...
BEGIN
  ...
  FOR m IN
    ...
    FROM regexp_matches(p_body, '@([\w\-\.]{2,40})', 'g') AS x
  LOOP
...
BEGIN
  ...
  IF d.author_id IS NOT NULL
     ...
     AND EXISTS (SELECT 1 FROM auth.users u WHERE u.id = d.author_id)
     AND NOT EXISTS (
       SELECT 1 FROM public.forum_notifications n
       WHERE n.user_id = d.author_id AND n.reply_id = NEW.id
     ) THEN
    INSERT INTO public.forum_notifications (user_id, actor_id, discussion_id, reply_id, kind, title, slug, excerpt)
  ...
  IF NEW.parent_reply_id IS NOT NULL THEN
    ...
    IF parent_author IS NOT NULL
       ...
       AND EXISTS (SELECT 1 FROM auth.users u WHERE u.id = parent_author)
       AND NOT EXISTS (
         SELECT 1 FROM public.forum_notifications n
         WHERE n.user_id = parent_author AND n.reply_id = NEW.id
       ) THEN
      INSERT INTO public.forum_notifications (user_id, actor_id, discussion_id, reply_id, kind, title, slug, excerpt)
...
AFTER INSERT OR UPDATE OF status ON public.forum_discussions
FOR EACH ROW EXECUTE FUNCTION public.forum_notify_on_discussion();

DROP TRIGGER IF EXISTS trg_forum_notify_on_reply ON public.forum_replies;
