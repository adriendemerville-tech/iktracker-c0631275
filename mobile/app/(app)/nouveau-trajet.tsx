...
export default function NouveauTrajet() {
  ...
  const importCalendar = async () => {
    ...
    if (!ok) {
      Alert.alert('Accès agenda refusé', 'Autorisez l’accès au calendrier pour pré-remplir vos trajets.');
      return;
    }
    const grouped = await fetchDayEvents();
    const flat = Object.values(grouped).flat();
    setEvents(flat);
    if (!flat.length) Alert.alert('Aucun rendez-vous', 'Pas de rendez-vous aujourd’hui dans vos agendas.');
  };
  ...
  const submit = async () => {
    ...
    if (!vehicle || !Number.isFinite(base) || base <= 0) {
      Alert.alert('Champs incomplets', 'Sélectionnez un véhicule et saisissez une distance valide.');
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TextInput style={styles.input} placeholder="Départ" value={startAddress} onChangeText={setStartAddress} placeholderTextColor={colors.muted} />
      <TextInput style={styles.input} placeholder="Arrivée" value={endAddress} onChangeText={setEndAddress} placeholderTextColor={colors.muted} />
      <TextInput style={styles.input} placeholder="Motif" value={purpose} onChangeText={setPurpose} placeholderTextColor={colors.muted} />
      <TextInput
        style={styles.input}
      ...
      <Pressable style={styles.primaryBtn} onPress={submit} disabled={saving}>
        <Text style={styles.primaryText}>{saving ? 'Enregistrement…' : 'Enregistrer le trajet'}</Text>
      </Pressable>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  chip: { borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, padding: spacing.md, backgroundColor: colors.surface },
  chipActive: { borderColor: colors.primary, backgroundColor: '#EEF0FF' },
  chipText: { color: colors.text },
  chipTextActive: { color: colors.primary, fontWeight: '600' },
  event: { borderLeftWidth: 3, borderLeftColor: colors.primary, paddingLeft: spacing.md, paddingVertical: spacing.sm },
  eventTitle: { color: colors.text, fontWeight: '600' },
  eventMeta: { color: colors.muted, fontSize: 12 },
  primaryBtn: { height: 52, borderRadius: radius.md, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center', marginTop: spacing.md },
  primaryText: { color: '#fff', fontWeight: '700', fontSize: 16 },
  secondaryBtn: { height: 48, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface },
