...
export default function TourneeScreen() {
  ...
  const requestPermissionsAndStart = useCallback(async () => {
    ...
    setPermissionStatus(
      await getLocationPermissionStatus().catch(() => ({
        foreground: "denied" as LocationPermissionStatus,
        background: "denied" as LocationPermissionStatus,
      })),
    );
    if (!ok) {
  ...
  const onStop = useCallback(async () => {
    ...
    if (!vehicle || result.distance <= 0) {
      Alert.alert(
        "Tournée terminée",
        `${result.distance.toFixed(1)} km — aucun trajet enregistré.`,
      );
      return;
    }
    try {
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      >
        <Text style={styles.btnText}>
          {running ? "Terminer la tournée" : "Démarrer la tournée"}
        </Text>
      </Pressable>
    </ScrollView>
  );
